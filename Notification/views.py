...
class check_and_notify_all_users_reminder(APIView):
    ...
    def get(self, request):
        ...
        for user in users:
            ...
            shifts = sc_roster.objects.filter(
                employee_id=employee.employee_id,
                confirmation__isnull=True,  # Filter shifts where confirmation is NULL
                shift_date__in=[current_date, next_day]
            )
            ...
            if shifts.exists():
                ...
                for shift in shifts:
                    if '-' in shift.shift_time:
                        ...
                        if shift.confirmation is None and  current_datetime <= shift_datetime_minus_4_hours  and shift_datetime_minus_8_hours <= current_datetime :
                            # Add to the filtered_shifts list if it meets the condition
                            filtered_shifts.append(shift)
                            serializer = ScRosterSerializer(shift)
                            shift_data = serializer.data
                            ...
                            notification_log_id = notification_entry.id
                            notification_result = send_push_notification(user, shift_data, notification_log_id)

                            if notification_result != "success":
                                error_details = notification_result.split("--")
                                if len(error_details) == 2:
                                    if error_details[1] == "Requested entity was not found.":
                                        notification_entry.notification_message = "App Is Not Installed"
                                    elif error_details[1] == "The registration token is not a valid FCM registration token":
                                    ...
                                    notification_entry.notification_message = notification_result

                                notification_entry.save()
                                errors.append(f"Error sending notification to {user.full_name} - {notification_result}")
                            else:
                                notification_entry.notification_received = timezone.now()
                                notification_entry.save()
                                success.append(f"Successfully sent notification to {user.full_name}")
